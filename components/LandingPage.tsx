    ...
    <div className="space-y-3 font-mono text-sm">
      {tasks.map((t, i) => (
        <div key={t.id} className={`flex items-center gap-3 p-3 rounded-lg border transition-all cursor-pointer ${t.completed ? 'bg-emerald-500/10 border-emerald-500/30 opacity-70' : 'bg-zinc-900 border-zinc-800 hover:border-violet-500/50'}`} onClick={() => toggle(t.id)}>
          ...
          {t.completed && <span className="ml-auto text-[10px] text-emerald-500 font-bold">+100 AURA</span>}
        </div>
      ))}
    ...
    <div className="min-h-screen bg-black text-white selection:bg-violet-500/30 selection:text-violet-200 font-sans overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/50 backdrop-blur-md border-b border-white/5">
        <div className="container mx-auto px-6 h-16 flex justify-between items-center">
          <div className="flex items-center gap-2 group cursor-pointer" onClick={onGetStarted}>
            ...
            <span className="text-lg font-bold tracking-tight font-mono">NEXUS</span>
          </div>
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 container mx-auto px-6 text-center">
        ...
        <p className="text-lg md:text-xl text-zinc-400 mb-10 max-w-2xl mx-auto font-light leading-relaxed font-mono">
          The AI productivity engine that understands your vibe. <br className="hidden md:block" />
          Turn your chaotic life into a gamified quest log.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-20">
          ...
          <button onClick={() => document.getElementById('features')?.scrollIntoView({behavior: 'smooth'})} className="text-zinc-400 hover:text-white text-sm font-mono flex items-center gap-2 px-6 py-4 rounded-xl border border-transparent hover:border-zinc-800 hover:bg-zinc-900 transition-all">
             <Command className="w-4 h-4" /> View Specs
          </button>
        </div>
      ...
      <section id="demo" className="container mx-auto px-6 py-24 border-t border-zinc-900">
        <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
                <h2 className="text-3xl font-bold font-mono uppercase tracking-tight mb-4 text-shadow-neon">Command Center</h2>
                <p className="text-zinc-400">Experience the interface before you join the server.</p>
            </div>

            <div className="bg-zinc-950 rounded-2xl border border-zinc-800 shadow-[0_0_50px_rgba(139,92,246,0.1)] overflow-hidden flex flex-col md:flex-row min-h-[600px]">
                {/* Simulator Sidebar */}
                ...
                <div className="flex-1 bg-black p-8 relative overflow-hidden">
                    ...
                    <div className="relative z-10 h-full flex flex-col">
                        ...
                        <div className="flex-1 bg-zinc-900/30 border border-zinc-800 rounded-xl p-6 backdrop-blur-sm shadow-2xl">
      ...
      <section id="features" className="container mx-auto px-6 py-24">
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[280px]">
            ...
            {/* The Yap Pad */}
            <div className="md:col-span-2 bg-zinc-900 rounded-3xl p-8 border border-zinc-800 relative overflow-hidden group hover:border-violet-500/30 transition-colors">
                ...
                <div className="relative z-10 h-full flex flex-col justify-between">
                    ...
                    <div className="flex gap-2 mt-4">
                        <div className="px-3 py-1 rounded bg-zinc-800 text-xs font-mono text-zinc-300 border border-zinc-700">Auto-Extraction</div>
                        <div className="px-3 py-1 rounded bg-zinc-800 text-xs font-mono text-zinc-300 border border-zinc-700">Sentiment Analysis</div>
                    </div>
                </div>
            </div>

            {/* Roast Mode */}
            <div className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800 flex flex-col justify-between group hover:border-red-500/30 transition-colors relative overflow-hidden">
                <div className="absolute inset-0 bg-red-500/5 group-hover:bg-red-500/10 transition-colors"></div>
                <div className="flex justify-between items-start relative z-10">
                    <Flame className="w-8 h-8 text-red-500" />
                    <span className="text-xs font-mono uppercase text-zinc-500">New Feature</span>
                </div>
            ...
            {/* Rot Mode */}
            <div className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800 flex flex-col justify-between group hover:border-zinc-500/30 transition-colors grayscale hover:grayscale-0">
                <div className="flex justify-between items-start">
                    <Ghost className="w-8 h-8 text-zinc-400" />
                    <span className="text-xs font-mono uppercase text-zinc-500">Wellness</span>
                </div>
                <div>
            ...
            {/* Visuals / Receipt */}
            <div className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800 flex flex-col justify-between group hover:border-pink-500/30 transition-colors">
                 <div className="flex justify-between items-start">
            ...
            {/* Aesthetics */}
            <div className="md:col-span-1 bg-zinc-900 rounded-3xl p-6 border border-zinc-800 flex flex-col justify-between group hover:border-cyan-500/30 transition-colors">
                 <div className="flex justify-between items-start">
                    <Palette className="w-8 h-8 text-cyan-400" />
                    <span className="text-xs font-mono uppercase text-zinc-500">Customization</span>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                  {[
                      ...
                      <div key={i} className="bg-black p-6 rounded-2xl border border-zinc-900 hover:border-zinc-700 transition-colors">
                          ...
                          <h3 className="font-bold mb-2 font-mono uppercase">{step.title}</h3>
      ...
      <footer className="container mx-auto px-6 py-24 text-center">
          <div className="max-w-2xl mx-auto">
            <h2 className="text-4xl md:text-5xl font-bold mb-8 tracking-tight font-mono uppercase text-shadow-neon">Ready to fix your life?</h2>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Button onClick={onGetStarted} variant="primary" className="h-12 px-8 w-full sm:w-auto font-mono uppercase tracking-widest">
                    Enter The Nexus
                </Button>
            </div>
